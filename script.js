function displayCanvas(){
    ...
    //Розрахунок координат центра и радіуса годинника
    ...
    //Малюємо рисочки годинника
    let radiusNum = radiusClock - 10; //Радіус знаходження рисочек	
    let radiusPoint;
    for(let tm = 0; tm < 60; tm++){
	  contextHTML.beginPath();
	  if(tm % 5 == 0){radiusPoint = 5;}else{radiusPoint = 2;} //для виділення годинних рисочок
    ...
    //Оцифровка циферблата годинника
...
//    Малюємо стрілки
    ...
    let d = new Date();                //Отримуємо екземпляр дати
    let t_sec = 6*d.getSeconds();                           //Знаходимо кут для секунд
    let t_min = 6*(d.getMinutes() + (1/60)*d.getSeconds()); //Знаходимо кут для хвилин
    let t_hour = 30*(d.getHours() + (1/60)*d.getMinutes()); //Знаходимо кут для годин
